...
/// and distributes the ASTRO between xADO and vxADO stakers.
///
...
///
/// * **ExecuteMsg::EnableRewards** Enables collected ASTRO (pre Maker upgrade) to be distributed to xADO stakers.
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn execute(
    deps: DepsMut,
    env: Env,
...
/// Swaps fee tokens to ASTRO and distribute the resulting ASTRO to xADO and vxADO stakers.
///
...
fn collect(
    ...
    // If no swap messages - send ASTRO directly to x/vxADO stakers
...
/// Distributes ASTRO rewards to x/vxADO holders.
///
/// ## Executor
/// Only the Maker contract itself can execute this.
...
/// Private function that performs the ASTRO token distribution to x/vxADO.
fn distribute(
    deps: DepsMut,
    env: Env,
    cfg: &mut Config,
...
///
/// * **staking_contract** address of the xADO staking contract.
///
/// * **governance_contract** address of the vxADO fee distributor contract.
///
/// * **governance_percent** percentage of ASTRO that goes to the vxADO fee distributor.
///

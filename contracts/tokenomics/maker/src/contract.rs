...
/// and distributes the ASTRO between ADO and vADO stakers.
///
...
///
/// * **ExecuteMsg::EnableRewards** Enables collected ASTRO (pre Maker upgrade) to be distributed to ADO stakers.
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn execute(
    deps: DepsMut,
    env: Env,
...
/// Swaps fee tokens to ASTRO and distribute the resulting ASTRO to ADO and vADO stakers.
///
...
fn collect(
    ...
    // If no swap messages - send ASTRO directly to x/vADO stakers
...
/// Distributes ASTRO rewards to x/vADO holders.
///
/// ## Executor
/// Only the Maker contract itself can execute this.
...
/// Private function that performs the ASTRO token distribution to x/vADO.
fn distribute(
    deps: DepsMut,
    env: Env,
    cfg: &mut Config,
...
///
/// * **staking_contract** address of the ADO staking contract.
///
/// * **governance_contract** address of the vADO fee distributor contract.
///
/// * **governance_percent** percentage of ASTRO that goes to the vADO fee distributor.
///
